...
internal static class Program
{
    ...
    static Channel Read(string path)
    {
        using (var stream = File.OpenRead(path))
        {
            ...
            Version parsed;
            if (value == null || !Version.TryParse(value.Version, out parsed) || !Regex.IsMatch(value.Version, "^[0-9]+\\.[0-9]+\\.[0-9]+$") || !Regex.IsMatch(value.Sha256 ?? "", "^[a-fA-F0-9]{64}$") || value.Length < 1 || value.ExePath != "versions/" + value.Version + "/PaperProgress.exe") throw new InvalidDataException("更新清单无效。");
            return value;
        }
    }
    static string CachedExe(string root, Channel value) { return Path.Combine(root, value.Version + "-" + value.Sha256.Substring(0, 12), "PaperProgress.exe"); }
    ...
    static void Main(string[] args)
    {
        ...
        var local = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "PaperProgress", "app-cache");
        ...
        try
        {
            ...
            var launcher = Path.Combine(shared, "PaperProgress.Launcher.exe");
        ...
        catch (Exception ex) { MessageBox.Show("无法启动论文进度。\n" + ex.Message, "论文进度", MessageBoxButtons.OK, MessageBoxIcon.Error); }
    }
}
