...
public sealed class NewPaperDialog : Window
{
    ...
    public NewPaperDialog()
    {
        ...
        var input = new TextBox { MaxLength = 500, Margin = new Thickness(0, 16, 0, 18) }; AutomationProperties.SetName(input, "论文标题"); body.Children.Add(input);
        ...
        var add = MainWindow.ActionButton("新增", () => { if (string.IsNullOrWhiteSpace(input.Text)) { input.Focus(); return; } PaperTitle = input.Text.Trim(); DialogResult = true; }, true); add.IsDefault = true; buttons.Children.Add(add); body.Children.Add(buttons);
