...
- **一键移到最前面**：点论文右下角齿轮 → “移到最前面”；极简模式在论文上右键。会切换为手动排序，把该论文移到当前列表开头，其余顺序保留；按优先级翻页时只调整当前页，不改变优先级。顺序会保存并同步，切换其他排序方式后按所选方式显示。
...
界面有中文和英文两版，在 设置 → 外观 → 界面语言 里切换。有新版本时窗口底部会出现一行提示：点"下载并安装"会先显示这一版改了什么，看清楚了再决定要不要装；装好重启后还能再看一次，不需要重新下载压缩包。在 设置 → 同步与启动 点击“现在检查一次”后，直接关闭或取消设置即可回到挂件下载，无需保存设置。
...
论文菜单、优先级菜单、极简菜单、托盘菜单、日期选择器、滚动条、悬停提示，以及新增、编辑、命名、确认和提示框的内容都跟随所选主题。Windows 文件选择器、颜色选择器和系统通知继续使用系统外观。
