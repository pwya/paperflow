...
应用只在**检查更新**时访问 GitHub，而且只读一个静态清单（`releases/latest/download/update.json`）。这一次请求是普通 GET：不带请求体、不带 Cookie、不带设备标识，也不带论文或设置里的任何内容；服务器能看到的是这次连接本身的 IP 和 User-Agent（`PaperFlow/<版本>`）。除此之外不使用任何网络 API，不发送遥测，也不保存云服务口令。设置里把“更新提示”选成**不提示**就一次联网都不做；下载新版本永远需要用户点“下载并安装”，程序不会静默安装。
...
当前版本不提供应用层加密，也没有代码签名。内建更新只在 HTTPS 下读清单、下载后校验长度与 SHA-256，装进 `versions/<版本>/` 并改写 `channel.json`（旧清单留一份 `.previous`）；校验不过的文件会被删掉，一个字节都不会覆盖现在的程序。
