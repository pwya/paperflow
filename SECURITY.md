...
应用本身不调用 GitHub、Notion 或其他网络 API，不发送遥测，也不保存云服务口令。OneDrive 客户端会按照用户自身账户设置同步安装目录内的私人资料。程序的“同步成功”不能证明云端已经收到文件。
...
设置里的完整备份导出含论文及本机设置，属于私人文件。OneDrive 不是独立备份，仍建议定期导出到另一个可信位置。
