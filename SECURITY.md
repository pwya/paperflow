...
当前版本不提供应用层加密，也没有代码签名。内建更新只在 HTTPS 下读清单、下载后校验长度与 SHA-256，装进 `versions/<版本>/` 并改写 `channel.json`（旧清单留一份 `.previous`）；校验不过的文件会被删掉，一个字节都不会覆盖现在的程序。哈希只能发现传输损坏和放错文件，不能证明发布者身份：能同时改程序和发布页的人仍然可以替换应用。只运行自己信任的发行包，介意这一点就自己从源码构建。
