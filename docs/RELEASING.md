...
只验证论文资料页时，在输出目录参数后追加 `--paper-details-only`，检查保存、取消、阶段勾选、方案切换及主题显示，不调用“显示桌面”。

追加 `--minimal-only` 可检查极简模式的设置保存与取消、小尺寸滚动、主题、优先级分页和资料编辑。`PaperFlow.exe --minimal-assets <输出目录>` 可导出两张极简模式与资料页的真实界面图，使用内置虚构论文，不读取个人资料。

追加 `--paper-management-only` 可检查两处删除入口、确认与取消、归档窗口的搜索、编辑、恢复及深色显示；仅操作临时目录中的虚构论文。删除使用版本 1 日志中的可选 `deleted=true` 记录，同一编号的删除永久生效；同时发送 `Archived=true`，让不认识删除字段的旧客户端先归档并保留记录。普通测试覆盖删除与旧编辑并发、重复及乱序投递、离线重启和写入失败。

追加 `--updates-only` 可用本地模拟更新服务器检查完整与极简模式：手动检查后保存、取消、关闭设置，以及先关闭再收到结果，均能看到更新提示；检查失败可重试，取消的偏好不保存，且不会自动下载。
