...
$root = Join-Path ([IO.Path]::GetTempPath()) ('PaperFlow-launcher-test-' + [guid]::NewGuid().ToString('N'))
$package = Join-Path $root 'package'
$cache = Join-Path $root 'cache'
$appFolder = Join-Path $package 'versions/1.0.0'
[IO.Directory]::CreateDirectory($appFolder) | Out-Null
$synthetic = Join-Path $appFolder 'PaperFlow.exe'
[IO.File]::WriteAllText($synthetic, 'Synthetic executable bytes; check-only mode never executes them.')
$channel = @{ version='1.0.0'; exePath='versions/1.0.0/PaperFlow.exe'; sha256=(Get-FileHash $synthetic).Hash.ToLowerInvariant(); length=(Get-Item $synthetic).Length }
...
$channel.version = '1.0.1'; $channel.exePath = 'versions/1.0.1/PaperFlow.exe'; Write-Channel
...
[IO.File]::WriteAllText((Join-Path $newFolder 'PaperFlow.exe'), 'Incorrect update bytes')
