...
while (malformed.Papers[0].Stages.Count > 1) malformed.Papers[0].Stages.RemoveAt(0);
Reject(() => storage.Save(malformed), "reject a paper with fewer than two stages");
...
// 2.0.0 起任意阶段都能标"不适用"，但完成与不适用仍然不能同时选中。
var anySkipped = Storage.CloneLibrary(library); anySkipped.Papers[0].Stages[0].Done = false; anySkipped.Papers[0].Stages[0].Skipped = true;
Storage.Validate(anySkipped); Check(anySkipped.Papers[0].Total == 5, "any stage may be marked not applicable");
var bothFlags = Storage.CloneLibrary(library); bothFlags.Papers[0].Stages[0].Skipped = true;
Reject(() => Storage.Validate(bothFlags), "done and not applicable cannot both be checked");
var duplicateStages = Storage.CloneLibrary(library); duplicateStages.Papers[0].Stages[1].Name = duplicateStages.Papers[0].Stages[0].Name;
Reject(() => Storage.Validate(duplicateStages), "duplicate stage names are refused");
var tooManyTags = Storage.CloneLibrary(library); tooManyTags.Papers[0].Tags = new() { "甲", "乙", "丙", "丁" };
Reject(() => Storage.Validate(tooManyTags), "a paper carries at most three tags");
var strangerTags = Storage.CloneLibrary(library);
strangerTags.Papers[0].Tags = new() { "等老师反馈" };
strangerTags.Settings.HiddenTags = new() { "等老师反馈", "没人提过的标签" };
Storage.Validate(strangerTags);
Check(strangerTags.Settings.HiddenTags.Count == 1, "hidden tags that nothing carries are dropped");
var customScheme = Storage.CloneLibrary(library);
customScheme.Settings.CustomSchemes = new() { new StageScheme("我的方案", new List<string> { "构思", "写作", "投稿" }) };
Storage.Validate(customScheme); Check(customScheme.Settings.CustomSchemes.Count == 1, "a scheme of your own is kept");
customScheme.Settings.CustomSchemes = new() { new StageScheme("标准七步", new List<string> { "甲", "乙" }) };
Reject(() => Storage.Validate(customScheme), "a scheme of your own cannot reuse a built-in name");
...
SyncTests.Run(Check);
SchemeTests.Run(Check);
WhatsNewTests.Run(Check);
